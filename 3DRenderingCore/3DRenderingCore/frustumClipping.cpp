...
Vector3D frustumClipping::intersectLineWithPlane(Vector3D pointOnPlane, Vector3D planeNormal, Vector3D pointOnLine, Vector3D lineDefinition) {
    ...
    if ((lineDefinition.dotProduct(planeNormal)) == 0) {
        return Vector3D(0, 0, 0); // This implies that there is no intersection, may require custom handling, though returning a false value will work in most circumstances
    }
...
std::vector<Vector3D> frustumClipping::getFrustumIntersections(Camera c, Vector3D frontpoint, Vector3D backpoint) {
    Vector3D pointOnPlane = c.position;
    ...
    std::vector<Vector3D> validIntersections;
    for (int i = 0; i < 4; i++) {
        double lineLength = visualTransform::relativize(backpoint, frontpoint).Magnitude();
        ...
        if (frustumClipping::isInFront(c, intersections[i]) && lineLengthError < 0.01) {
            validIntersections.push_back(intersections[i]);
