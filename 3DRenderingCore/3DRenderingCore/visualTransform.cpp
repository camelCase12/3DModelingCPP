...
double visualTransform::angleBetweenVectors(Vector3D v1, Vector3D v2) {
	double parameter = v1.dotProduct(v2) / (v1.Magnitude() * v2.Magnitude());
	if (parameter >= 1) return 0;
	else if (parameter <= -1) return M_PI;
	return acos(parameter); // Classic LinAlg formula, the dot product over the product of the magnitudes equals to the cos of the angle between.
}

double visualTransform::angleBetweenVectors(Vector2D v1, Vector2D v2) { // for some ungodly reason, these two functions ocassionaly feed the acos with a value that is infinitesimally above 1?
	double parameter = v1.dotProduct(v2) / (v1.Magnitude() * v2.Magnitude());
	if (parameter >= 1) return 0;
	else if (parameter <= -1) return M_PI;
	return acos(parameter); // translated from 3d to 2d
}
...
Vector2D visualTransform::relativize(Vector2D zero, Vector2D relative) {
	return Vector2D(relative.x - zero.x, relative.y - zero.y); // Subtracts each component of the two vectors, zeroing out the relative with respect to the first
}
...
Vector2D visualTransform::projectVector(Vector2D a, Vector2D b) { // re-implementation of Vector3D proj
	double numeratorScalar = a.dotProduct(b);
	double denominatorScalar = b.dotProduct(b);
	double fullScalar = numeratorScalar / denominatorScalar;
	return b.scalarMultiply(fullScalar);
